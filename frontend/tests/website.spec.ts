...
test('overview, precise tooltip, detail layout, inputs, generation selector and downloads', async ({ page, isMobile }) => {
  ...
  await expect(page.getByRole('button', { name: 'Next available day' })).toBeDisabled()
...
test('date arrows skip missing dates and historical observations never acquire forecasts', async ({ page }) => {
  await mockApi(page)
  await page.goto('/')
  await page.getByRole('button', { name: 'Previous available day' }).click()
  ...
  await expect(page.getByRole('button', { name: 'Previous available day' })).toBeDisabled()
