  ...
  return <main className="detail-page sources-page">
    ...
    <section className="source-section" aria-labelledby="market-source">
      ...
      <div className="source-copy">
        ...
        <p>For each delivery day, the model uses that day’s EXAA prices, earlier SDAC prices, load and generation forecast curves from the previous day, and measured load and generation from two days earlier. Prices are expressed in EUR/MWh; load and generation in MW.</p>
    ...
    <section className="source-section" aria-labelledby="weather-source">
      ...
      <div className="source-copy">
        ...
        <p>DELU aligns shortwave radiation to the preceding hour it represents, then expands hourly values into quarter-hour features. The weather charts and table show arithmetic means across the 25 locations; the model uses each location separately. These are DELU transformations of the supplied forecasts.</p>
    ...
    <section className="source-section" aria-labelledby="delu-processing">
      ...
      <div className="source-copy">
        <p>DELU aligns source dates, derives residual load, and normalizes each local day to 96 quarter-hour positions after validating its physical grid. At daylight-saving transitions, repeated scalar quarters are averaged, wind direction uses a circular mean, and missing spring clock positions are filled from adjacent values. The downloadable Gold dataset contains these prepared values.</p>
        <p>The model learns a correction to EXAA prices and produces a price forecast with a 90% target prediction interval. Forecasts, intervals, and performance metrics are generated by DELU. Attribution does not imply that any data provider endorses the model or its results.</p>
        <p>Missing source data stays pending and is retried automatically. Weather is fetched when its run becomes available. Published forecasts keep their original values and creation timestamps. Evaluation uses the 92, 96, or 100 physical delivery intervals; forecasts created after the previous day’s 12:00 Europe/Berlin cutoff are marked retrospective and excluded from operational rolling monitoring.</p>
