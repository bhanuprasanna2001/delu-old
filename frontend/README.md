...
Share a selected day with `?date=YYYY-MM-DD&view=detail`. The date index refreshes
every 15 minutes, pending forecasts every five minutes, and settled forecasts and
evaluation metrics every 30 minutes. Publication timestamps remain visible;
model-quality alerts are shown once in the performance section. The website uses
real API data; synthetic data is limited to the browser tests.
